...
pub struct DaManager<T> {
    data: Mutex<BTreeMap<SH256, DaItem<T>>>,
}
...
impl<T> DaManager<T> {
    ...
    fn clean(&self, raw: &mut BTreeMap<SH256, DaItem<T>>) {
    ...
    pub fn get(&self, hash: &SH256) -> Option<Arc<T>> {
        let mut data = self.data.lock().unwrap();
        data.get(hash).map(|n| n.get()).flatten()
    }

    pub fn put(&self, hash: SH256, raw: Arc<T>, alive_secs: u64) {
    ...
    pub fn try_lock(&self, hashes: &[SH256], alive_secs: u64) -> Vec<DaItemLockStatus> {
        ...
        for hash in hashes {
            match data.entry(*hash) {
                ...
                Entry::Vacant(entry) => {
                    let da_item = entry.insert(DaItem::lock(alive_secs));
