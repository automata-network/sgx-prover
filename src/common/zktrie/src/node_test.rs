...

    {
        ...
        match node.value() {
            NodeValue::Branch(node) => {
                ...
                assert_eq!(BranchHash::Ternimal(r), node.right);
            }
            _ => unreachable!(),
        }
        assert_eq!(
...
fn test_new_node_from_bytes() {
    ...
    type Node = crate::Node<TestHash>;
    {
        ...
        match node.value() {
            NodeValue::Branch(node) => {
                assert_eq!(node.left, BranchHash::Ternimal(k1));
                assert_eq!(node.right, BranchHash::Ternimal(k2));
            }
            _ => unreachable!(),
        }
        assert_eq!(
...
fn test_node_value_and_data() {
    ...
    assert_eq!(
        ...
        &[
            0x9, 0x0, 0x1, 0x61, 0x61, 0x61, 0x61, 0x61, 0x61, 0x61, 0x61, 0x61, 0x61, 0x61,
            0x61, 0x61, 0x61, 0x61, 0x61, 0x61, 0x61, 0x61, 0x61, 0x61, 0x61, 0x61, 0x61, 0x61,
            0x61, 0x61, 0x61, 0x61, 0x61, 0x0, 0x1, 0x61, 0x61, 0x61, 0x61, 0x61, 0x61, 0x61,
            0x61, 0x61, 0x61, 0x61, 0x61, 0x61, 0x61, 0x61, 0x61, 0x61, 0x61, 0x61, 0x61, 0x61,
